...
export class AwsProxy {
    ...
    async signUrl(request, method = null) {
        ...
        url.search = [...url.searchParams]
            ...
            .map(pair => pair.join('='))
            .join('&')
        return new Request(url, signedRequest)
    }
    ...
    async fetch() {
        ...
        if (url.searchParams.has('X-Amz-Signature')) {
            //Got a signed url, we use same way to take advantage of X-Amz-Expires
        ...
        if (headersToSign['x-amz-content-sha256'] === 'STREAMING-AWS4-HMAC-SHA256-PAYLOAD') {
            ...
            if (headersToSign['content-length']) {//Update content-length header if present
